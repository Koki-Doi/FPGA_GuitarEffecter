...
Status: **survey executed; Compressor fixed in D125 and superseded by D131**
(updated 2026-06-15). This started as a measurement survey on the D124 baseline;
the Compressor RATIO issue was fixed by D125 and bench-ACCEPTED. Wah and Noise
Suppressor remain on-target; Reverb and preset loudness remain bench/final-chain
items. `REALISM_IMPROVEMENT_WORK_ORDER.md`
...
| 8 | Compressor | **RATIO knob ほぼ無効 + 圧縮が浅い** | **D125 で修正、bench-ACCEPTED、D131 に supersede** |
...
### 修正 = D125（Dyna/Ross sustain、bench-ACCEPTED）
...
（bit ロード、ADC HPF True、compressor GPIO 健在）。この時点では ear-bench 待ちだったが、
後続で bench-ACCEPTED され、D125 が一時 canonical baseline になった。
