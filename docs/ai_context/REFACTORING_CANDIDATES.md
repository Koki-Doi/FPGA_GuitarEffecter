...
| --- | --- | --- | --- | --- |
| **P1** | `AudioLabOverlay.py` (1893 lines) is the largest module. `set_guitar_effects` is already split into 6 private helpers (D-era). Next: extract per-effect register writers (overdrive / distortion / amp / cab / eq / reverb) into an `overlay/` subpackage with the class as a thin facade (the `hdmi_state/` + `compact_v2/` split pattern). | `audio_lab_pynq/AudioLabOverlay.py` | High (most-touched API) | Keep the public method surface + snapshot-test byte output identical. |
