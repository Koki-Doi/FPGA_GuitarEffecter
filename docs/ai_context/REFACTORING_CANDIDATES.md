...
Remaining DSP: **D/B2** (Pipeline combinators, deferred -- high-risk Signal-level).
...
| **B** | Shared biquad kernels `biquadFf` / `biquadRec` / `biquad5` in `FixedPoint`, dedup the resonant tone biquads. Pipeline x1/x2/y1/y2 wiring + D82 ff/rec split unchanged. | `FixedPoint.hs` + biquad sites | Med | Low (VHDL-proven == D102; note Vivado may repack DSP48: D103 saw DSP 137->139) | **DONE 2026-06-22** (`refactor/b-biquad-kernels`). 3 kernels added; deduped 6 sites: TS mid hump + transformer resonance (`biquad5` single-stage), Big Muff/DS-1/Metal scoop + Fuzz Face mid + OD mid + amp scoop mux (`biquadFf` + `biquadRec` ff/rec split). The two `- a1*y1` sites (OD mid, amp scoop) pass `negate a1` into the unified `+ na1*y1` kernel (value-identical by mulS16 linearity). golden 20/20 byte-identical; vhdl/bit LEFT at D155 (F-pattern -- the NEXT bit rebuild from this source gets the usual timing/CDC/ear-bench, and may repack DSP48 per D103). |
