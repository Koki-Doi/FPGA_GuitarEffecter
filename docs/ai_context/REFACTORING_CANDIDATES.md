...
Remaining DSP: **B** (biquad kernels), **K** (split Pedals.hs), **D/B2** (Pipeline
combinators, deferred). Remaining Python: **P1** orchestration tail, **P2**
renderer panel split, **P3**, **M** (hdmi mirror class audit).

**2026-06-22: J DONE** -- clip-family shift parametrization (`softClipShift` +
5 wrappers in `FixedPoint.hs`); golden regression 20/20 byte-identical, vhdl/bit
LEFT at D155 (F-pattern, no bench for the merge). New candidates K / L / M recorded.
...
| **K (new)** | Split `Distortion/Pedals.hs` (583 lines -- now the LARGEST Clash source after the F split). The per-pedal stages (DS-1 / Big Muff / Metal / Fuzz Face / MT-2) sit in one file; split into `Distortion/Pedals/{BigMuff,Ds1,Metal,Fuzz,...}.hs` behind a re-export shim (F / D26 pattern). | `Effects/Distortion/Pedals.hs` | Med | **Low** (pure code-move; golden) | Pending. |
...
| **M (new)** | `audio_lab_pynq/hdmi_effect_state_mirror.py` is still **1238 lines** (the second-largest Python module) even after the per-effect split under `hdmi_state/`. Audit whether more of the `HdmiEffectStateMirror` class body (per-effect mirror/apply logic) can move to `hdmi_state/` with the class keeping thin delegates (the established split pattern), or whether the residual is genuinely class-coupled. | `audio_lab_pynq/hdmi_effect_state_mirror.py`, `audio_lab_pynq/hdmi_state/` | Med | Verify with `tests/` + an HDMI-state spot check; no bitstream / bench. Pending. |
