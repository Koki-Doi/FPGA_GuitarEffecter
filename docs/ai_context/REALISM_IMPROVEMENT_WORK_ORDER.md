...
Status: **work order CONVERGED at D135 (`533d5869`)**
(updated 2026-06-19). D121 以降で AudioLab を実機らしい方向へ寄せるための作業順と
...
Amp/Cab character を追加して bench-ACCEPTED。残る主対象は real-IR cab
convolution と、safe-bypass CDC を物理的に安定化した後の chord-detune 再検討。
D144 は offline sim では改善したが bench-rejected で D135 へ rollback 済み。
...
- 現在の canonical deployed voicing baseline は **D135**（merge `765323b`、bit
  `533d586901dc3669285a49c6d82bab9f`、hwh
  `731517487c6218f0e181c2b74485d7a6`）。この work order は D121 時点から
  始まった計画なので、下の D121 箇条書きは初期 voicing pass の記録として残す。
...
- D122-D135 で Amp/Cab/RAT/Compressor/OD/DS/DIST の追加測定修正と
  all-effect objective coverage が bench-ACCEPTED され、D135 が現在の
  canonical baseline。
