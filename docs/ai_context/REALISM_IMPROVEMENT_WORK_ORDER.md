...
Status: **work order CONVERGED at D135 (`533d5869`); current baseline D155 (`09c8a95`)**
(updated 2026-06-22). D121 以降で AudioLab を実機らしい方向へ寄せるための作業順と
測定方針をまとめた計画書。Cab / OD / Distortion / Compressor / Amp の低リスク
voicing pass は D121-D131 で実装・bench-ACCEPTED。2026-06-17 の all-effects
sim survey 以降で、低音(amp HP dead-pole→live)、こもり(HF-restore)、Metal
完全飽和(os4x floor 600k)、RESONANCE 死にノブ、クリーン用パワーヘッドルーム
(per-model `ampPowerKnee`)を実装・bench-ACCEPTED し、sim を包括検知器
(muffled/harsh・clean-distortion・全モデルターゲット)に強化。**全モデルが実機
ターゲットに一致(28/28 EQ + 7/7 歪 + 6/6 amps-clean、`REALISM_ALL_EFFECTS_SIM_SURVEY.md`)。**
D134 で dynamics / knob coverage を拡張し、D135 で Fuzz Face mid-hump と
Amp/Cab character を追加して bench-ACCEPTED。D144 は offline sim では改善したが
bench-rejected で D135 へ rollback 済み。その後 D146 で audio-output CDC を
hard pblock（`SLICE_X100Y116:SLICE_X113Y137`）で物理固定し、D147 amp sag-attack
slew と D148 JC/Twin clean-headroom fix（`clip_onset.py` で局在化）が bench-ACCEPTED
され、その後 D149-D155 の voicing アーク（OD/DS 対称クリップ・amp 高音・cab 47-tap FIR 等）が land、現行 baseline は **D155**（`09c8a95`）。残る主対象は full 128-tap real-IR cab convolution。
...
- `REALISM_CAB_MEASUREMENT.md`: step 4 の成果物。D121 Cab 3 model 実測 + target 差分。
  phase 3 = D123 per-model presence biquad（bench-ACCEPTED, merged）。
- `REALISM_OD_DIST_MEASUREMENT.md`: steps 5/6 の成果物。OD 6 + Dist 7 全測定。
  RAT が全 drive で THD 0%（dead-pole HP）= D124 で修正（bench-ACCEPTED, merged）。
  他は on-target。
- `REALISM_DYNAMICS_REVERB_MEASUREMENT.md`: steps 7-11 サーベイ。Wah/NS on-target、
  Compressor の RATIO 無効（次の DSP 修正候補）、Reverb/loudness は耳/全chain依存で後工程。
- `REAL_HARDWARE_REFERENCE_ALIGNMENT_PLAN.md`: ユーザー提示の BD-2 / TS / SD-1(OD-1
  要確認) / DS-1 / Centaur / RAT / Fender / Marshall 資料を、実装前の測定 target と
  低リスク候補へ落とした計画書。
- `REALISM_REFERENCE_ALIGNMENT_FINDINGS.md`: 上記資料を measured target に落とし、
  D126/D127 で実行した最初の reference-alignment pass と、その後 D128-D131 で
  追加実装した OD/DS/RAT/Amp/DIST realism line の起点。
- `tools/dsp_sim/dist_eval.py` / `tools/dsp_sim/targets.py`: D131 で追加した
  distortion-character / target-check tooling。
...
- 現在の canonical deployed voicing baseline は **D155**（merge `09c8a95`、bit
  `8d875cc8a0154a86673ab22e5b142d27`、hwh
  `e0469cf593e97d582c14bb09ea98d3d3`）= cab speaker FIR 31->47 taps、D150-D155 アークの締め。基盤は JC/Twin clean-headroom fix + D146 hard
  CDC pblock + D147 sag slew、D135（`765323b`）を supersede。この work order は
  D121 時点から始まった計画なので、下の D121 箇条書きは初期 voicing pass の記録
  として残す。
...
- D122-D135 で Amp/Cab/RAT/Compressor/OD/DS/DIST の追加測定修正と
  all-effect objective coverage が bench-ACCEPTED され、その後 D146-D148 で
  CDC hard pblock + sag slew + JC/Twin clean-headroom fix を重ねて D148 が現在の
  canonical baseline。
