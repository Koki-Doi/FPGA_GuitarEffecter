# Resume prompts — history

Phase-specific resume prompts that were active during the HDMI GUI
Phase 1 -- Phase 6H arc and during each DSP / Notebook deploy that
shipped along the way. Kept verbatim so a future agent asked to
revisit one of those efforts has the same context the original
session did. The currently-active prompts (general resume, the
Phase 6I C2 SVGA 800x600 HDMI baseline, PYNQ deploy, adding an
effect, etc.) live in [`RESUME_PROMPTS.md`](RESUME_PROMPTS.md).

When a section here contradicts the current baseline (e.g. earlier
Phase 6F/6G/6H HDMI sections still describe the rejected native
800x480 / 40 MHz timing), trust `DECISIONS.md` D25, `CURRENT_STATE.md`,
and `HDMI_GUI_INTEGRATION_PLAN.md` Section 11 over the historical
prompt.

## HDMI GUI integration — pre-Phase-6I subsections
