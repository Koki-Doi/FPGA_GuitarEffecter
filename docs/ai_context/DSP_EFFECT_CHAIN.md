...
| Stage | What it does |
| --- | --- |
| `wahPosSmooth` (`wahPosSmoothNext` register feedback) | Pedal-position zipper smoother. `posSmooth' = posSmooth + ((target - posSmooth) >> 5)` per audio frame (D98 96 kHz: >>5, was >>4 at 48 kHz, so the smoothing time stays ~constant in ms), with a 1-step nudge so single-byte targets still converge. Snaps to target on off-cycles so re-enable is clean. |
