...
Clash DSP pipeline adds only ~3 us (~0.31 samples at 96 kHz). So most "make the DSP
shorter" ideas have negligible payoff; the one real lever is the **sample rate**.

## Latency budget (current D98+ path at 96 kHz; 1 sample = 10.42 us)

| Stage | Estimate | Reducible? | Notes |
| --- | --- | --- | --- |
| **CS5343 ADC group delay** | ~0.1-0.25 ms (scaled estimate, ~tens of samples) | only via fs or a different codec | The ADC decimation filter. Fixed by the part + sample rate. The dominant contributor with the DAC. |
| I2S framing in (i2s_to_stream) | ~1 sample (~10 us) | no (protocol) | One stereo frame to assemble a sample. |
...
| **Clash DSP pipeline** | **~106 register stages ≈ 3.2 us @ 33.33 MHz ≈ 0.31 sample** | marginally | 106 `register Nothing` data stages. Streams 1 sample/cycle; a sample traverses in ~106 island cycles, far under one 96 kHz period. **Essentially not a latency factor.** |
| `cc_dsp_out` AXIS clock converter (33->100 MHz) | a few cycles | no | |
| D50 `mode2_right_snapshot` | ~1 frame (~10 us) | yes, but needs a bug fix | The mode-2 mono RIGHT-slot mirror deliberately delays one frame to dodge the i2s_to_stream LEFT-extraction bug + i2sOut race (CLAUDE.md D50). |
| I2S framing out + **CS4344 DAC group delay** | ~1 sample + ~0.1-0.25 ms | only via fs / codec | The DAC interpolation filter group delay -- the other dominant contributor. |
| **Estimated round-trip total** | **~0.35-0.8 ms** | — | Scaled estimate after the D98 96 kHz conversion; measure on the board before treating it as a specification. |
...
The DSP island runs at 33.33 MHz but only accepts one valid sample per 96 kHz period
...
cycles ≈ **3.2 us**, i.e. ~0.31 of a sample period. So even though the chain is
...
### 1. Sample rate 48 -> 96 kHz  [DONE at D98; biggest lever]
...
### 3. Remove the D50 mode-2 one-frame snapshot delay  [LOW priority, ~10 us]

- The `mode2_right_snapshot` mono mirror costs ~1 frame (~10 us at 96 kHz). Removing it
...
The round-trip latency is expected to be **below ~1 ms and codec-bound**, so
there is no cheap DSP-side win. The honest options are:

- **The main latency reduction is already deployed:** D98 moved the path to
  **96 kHz** and re-voiced every fs-dependent constant.
