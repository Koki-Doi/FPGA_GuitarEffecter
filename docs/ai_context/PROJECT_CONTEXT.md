...
| `audio_lab_pynq/effect_catalog.py` | Pure Python catalog for GUI-visible effects, knob layouts, model labels, model index tables, and chain-preset display names. `GUI/compact_v2/knobs.py` and `audio_lab_pynq/hdmi_state/{knobs,pedals,amps,cabs}.py` are compatibility import paths over this catalog. |
| `audio_lab_pynq/app_state_apply_plan.py` | Pure AppState -> `AudioLabOverlay` call planner shared by `GUI/audio_lab_gui_bridge.py` and `EncoderEffectApplier`. Owns knob extraction, EQ 0..100 -> overlay 0..200 scaling, Cab model handling, bridge dry-run plans, and encoder RAT-skip pedal-mask generation. |
| `audio_lab_pynq/hdmi_effect_state_mirror.py` | Compatibility shim for the notebook -> overlay -> HDMI GUI state mirror. It re-exports `HdmiEffectStateMirror` and helper names from `audio_lab_pynq/hdmi_state/mirror.py` so every historical import path (`from audio_lab_pynq.hdmi_effect_state_mirror import X`) keeps working. |
| `audio_lab_pynq/hdmi_state/` | Per-effect / per-theme submodules used by `hdmi_effect_state_mirror`. `mirror.py` holds the `HdmiEffectStateMirror` implementation; `pedals.py` / `amps.py` / `cabs.py` carry the model names + labels + aliases for each category; `selected_fx.py` holds the SELECTED FX classification + dropdown chip plumbing; `knobs.py` holds `GUI_EFFECT_KNOBS` (the 8-slot per-effect knob layout); `resource_sampler.py` holds the `/proc`-based `ResourceSampler` + `STATIC_PL_UTILIZATION`; `common.py` holds the shared `_clamp_percent` / `_model_key` / `_normalize_index_or_name` helpers; `frame_analysis.py` holds offline strong-UI bbox analysis shared by tests and HDMI diagnostics. |
...
| `GUI/pynq_multi_fx_gui.py` | 800x480 compact-v2 HDMI GUI renderer (`render_frame_800x480_compact_v2`). After the post-Phase-6I split (`DECISIONS.md` D26), this is a thin re-export shim over `GUI/compact_v2/`; the actual palette / themes / `AppState` / render functions / `hit_test_compact_v2` live in `GUI/compact_v2/{knobs,state,layout,renderer,render_compact_v2,hit_test}.py`. Every `from pynq_multi_fx_gui import X` import (including `AppState`, `EFFECT_KNOBS`, `render_frame_800x480_compact_v2`, `compact_v2_panel_boxes`, `hit_test_compact_v2`, etc.) keeps working unchanged. After the Phase 6H (1).py spec port (`d7ea0ab`), `EFFECT_KNOBS` is keyed by title-case `EFFECTS` names, `AppState` uses `all_knob_values`, and PEDAL / AMP / CAB draw the model dropdown inline. The 1280x720 reference renderer / Tk desktop preview / `run_pynq_hdmi()` were removed in `DECISIONS.md` D24. |
| `GUI/compact_v2/` | Per-theme split of the compact-v2 GUI: `knobs.py` (per-effect knob layout + model tables + chain preset names), `state.py` (`AppState` + `save_state_json` / `load_state_json`), `layout.py` (palette + themes + `COMPACT_V2_LAYOUT` + `compact_v2_panel_boxes`), `renderer.py` (NumPy / Pillow render primitives + `RenderCache` + public `render_frame_800x480_*` wrappers), `render_compact_v2.py` (large compact-v2 800x480 drawing routine), `hit_test.py` (`hit_test_compact_v2`). |
| `GUI/audio_lab_gui_bridge.py` | Dry-run-first bridge from GUI `AppState` to existing `AudioLabOverlay` control APIs. Thin executor over `audio_lab_pynq/app_state_apply_plan.py`; it no longer carries a private copy of GUI constants or AppState-to-overlay conversion logic. |
