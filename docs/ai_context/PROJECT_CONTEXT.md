...
| `audio_lab_pynq/encoder_ui.py` | `EncoderUiController` maps `EncoderEvent` instances into compact-v2 `AppState` mutations. D47 mapping: Encoder0 rotate selects the effect and Encoder0 short toggles it; Encoder1 rotate selects a knob, while Encoder1 hold+rotate cycles Distortion / Overdrive / Amp Sim / Cab IR models; Encoder2 rotate changes the selected knob value. Phase 7G+ `applier=` / `live_apply=` / `skip_rat=` still route overlay writes through `EncoderEffectApplier`; legacy `mirror=` / `bridge=` fall-through is preserved when no applier is supplied. |
