...
| `audio_lab_pynq/AudioLabOverlay.py` | Python facade: overlay loading, AXIS routing, public control APIs, and compatibility delegates for control-word/register-write helpers. The GPIO byte shapes live in `control_maps.py`; register-write/cache helpers are split under `audio_lab_pynq/overlay/` (D115). |
...
| `audio_lab_pynq/encoder_effect_apply.py` | Phase 7G+ `EncoderEffectApplier`: the only Python object allowed to translate compact-v2 `AppState` into `AudioLabOverlay` writes from the encoder runtime. Uses `set_noise_suppressor_settings`, `set_compressor_settings`, and `set_guitar_effects(**kwargs)` only — no raw GPIO writes. Default throttle 100 ms. RAT is GUI-selectable as of D91 (entry points default `skip_rat=False`; selecting RAT routes the Distortion knobs to the dedicated RAT stage); the constructor default stays `skip_rat=True` (library-safe), and `skip_rat=True` excludes pedal-mask bit 2 from cycling / live apply. Tracks `apply_count` / `error_count` / `last_apply_ok` / `last_apply_message` / `unsupported` for the GUI status strip and resource print. See `DECISIONS.md` D37 / D91. |
...
| `audio_lab_pynq/overlay/` | Implementation helpers behind the `AudioLabOverlay` facade. `register_writers.py` owns shared register-write/cache-refresh helpers for Distortion / Overdrive / Noise Suppressor / Compressor / Wah / full-chain writes while preserving existing `AudioLabOverlay` method names as delegates (D115). |
