...
- `docs/ai_context/PMOD_I2S2_INTEGRATION_PLAN.md` (current PMOD JB audio owner)
...
| **Encoder 3** | selected parameter の値を変更 (live\_apply=True なら `EncoderEffectApplier` 経由で throttled apply) | 決定 / 強制 apply (`apply_pending` 反映、live\_apply に関係なく force) | 選択中 knob を GUI default に戻す + apply |
...
- 使う overlay API は `set_noise_suppressor_settings`、
  ...
  整合)。
- throttle は `apply_interval_s`。`EncoderEffectApplier` class default は
  100 ms だが、現行 runner (`scripts/run_encoder_hdmi_gui.py`) は D76 latency
  fix 後の既定として 20 ms を渡す。連続回転で flooding しない。encoder 3
  short press は throttle を bypass して force apply。
- RAT: **D91 以降は GUI で選択可能（既定 `skip_rat=False`）**。エントリ
  ポイント (`run_encoder_hdmi_gui.py --include-rat`、`EncoderGuiSmoke.ipynb` /
  `PmodI2S2HdmiGuiOneCell.ipynb`) が RAT を有効化する。pedalboard の RAT slot
  (`distortion_pedal_mask` bit 2) は DSP no-op で、本物の RAT は専用上流ステージ
  ＝`set_guitar_effects` が rat bit を見て `rat_on=True` を自動 assert する。RAT
  選択時、applier は GUI の Distortion ノブを RAT ステージへ配線
  (TONE→`rat_filter` / LEVEL→`rat_level` / DRIVE→`rat_drive` / 6番目→`rat_mix`)。
  `skip_rat=True` (CLI `--skip-rat`) は従来通り RAT を巡回/apply から除外できる
  (applier/UI コンストラクタの既定は `skip_rat=True` のままで、エントリポイント
  だけ opt-in)。`DECISIONS.md` D91。
...
- 現行 Pmod I2S2 audio module、HDMI、encoder 配線の同居・grounding 検証

---

## 6. Phase 状態

### Phase 7A / 7B (historical planning)
...
- DSP / Clash は **未変更**
- 本ドキュメント + `IO_PIN_RESERVATION.md` + `EXTERNAL_PCM1808_PCM5102_AUDIO_PLAN.md` の 3 本 + `CURRENT_STATE.md` / `DECISIONS.md` / `RESUME_PROMPTS.md` の追記のみ。当時の PCM5102 / PCM1808 前提は D48 以降 Pmod I2S2 に置き換わった。
