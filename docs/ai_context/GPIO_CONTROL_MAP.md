...
| `axi_gpio_amp_tone` | `0x43CA0000` | amp simulator tone stack | bass | middle | treble | bit-packed: ctrlD[7] = amp_drive_mode, ctrlD[6:3] reserved, ctrlD[2:0] = amp_model_idx | active / active / active / active | D55 widens the amp model field from 2 bits to 3 bits so six researched voicings fit. `ctrlD[2:0]` selects the amp model (0=JC-120, 1=Twin Reverb, 2=AC30, 3=Rockerverb, 4=JCM800, 5=TriAmp Mk3; 6..7 reserved -> Clash falls back to 0=JC-120 as a safety default). `ctrlD[7]` is the binary Clean/Drive switch consumed directly by `Amp.hs` (`ampModelIdxF :: Unsigned 3` / `ampDriveModeF`); `ctrlD[6:3]` is reserved (Python always writes 0). The Python writer is `AudioLabOverlay.amp_model_drive_byte(amp_model_idx, amp_drive_mode)` with `AMP_MODEL_IDX_MASK = 0x07` and `AMP_MODEL_IDX_MAX = 5` (the writer clamps idx to 5 before pack; the older `amp_character_byte_for_model` name is preserved as an alias). Drive mode is a real DSP branch (per-model knee deltas + harder negative-side shift + per-model pre-LPF darken + per-model second-stage gain bonus + per-model treble / presence trims). See `AMP_MODEL_RESEARCH_D55.md` for the voicing rationale. The legacy `amp_character` percent kwarg of `set_guitar_effects` still works when `amp_model_idx` is not supplied (chain presets / older notebooks). |
