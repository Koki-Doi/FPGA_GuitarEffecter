...
| `axi_gpio_amp_tone` | `0x43CA0000` | amp simulator tone stack | bass | middle | treble | bit-packed: ctrlD[7] = amp_drive_mode, ctrlD[6:2] reserved, ctrlD[1:0] = amp_model_idx | active / active / active / active | D54 redefines `ctrlD` from a single character byte to a two-field bit-pack: `ctrlD[1:0]` selects the amp model (0=jc_clean, 1=clean_combo, 2=british_crunch, 3=high_gain_stack); `ctrlD[7]` is the binary Clean/Drive switch consumed directly by `Amp.hs` (`ampModelIdxF` / `ampDriveModeF`); `ctrlD[6:2]` is reserved (Python always writes 0). The Python writer is `AudioLabOverlay.amp_model_drive_byte(amp_model_idx, amp_drive_mode)` (the older `amp_character_byte_for_model` name is preserved as an alias). The pre-D54 in-band character shift (D53) is retired -- drive mode is now a real DSP branch in `Amp.hs` (lower clip knees + harder negative-side shift + extra pre-LPF darken + second-stage gain bonus). The legacy `amp_character` percent kwarg of `set_guitar_effects` still works when `amp_model_idx` is not supplied (chain presets / older notebooks). |
