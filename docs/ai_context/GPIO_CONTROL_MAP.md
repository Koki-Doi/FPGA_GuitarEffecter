...
| `axi_gpio_amp_tone` | `0x43CA0000` | amp simulator tone stack | bass | middle | treble | character (model-derived + drive-mode-shifted) | active / active / active / active | After D53 `ctrlD` is no longer a user-facing continuous CHAR knob: the Python `AudioLabOverlay.amp_character_byte_for_model(amp_model_idx, amp_drive_mode)` picks one of four labelled centre bytes (26 / 89 / 153 / 216 for jc_clean / clean_combo / british_crunch / high_gain_stack) and adds `AMP_DRIVE_MODE_OFFSET` (=30) when the new binary `amp_drive_mode` field is 1, keeping the byte inside the same Clash `ampModelSel` band (cutpoints at 63 / 126 / 190). `amp_drive_mode=0` produces the byte the pre-D53 percent-only path produced for each labelled model, so existing bitstreams are bit-for-bit compatible. The legacy `amp_character` percent kwarg of `set_guitar_effects` still works when `amp_model_idx` is not supplied (chain presets / older notebooks). |
