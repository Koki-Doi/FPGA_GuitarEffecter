...
Latest work: **2026-05-24 (D66 DS-1-only asymSoftClip knee retune accepted and deployed):** Branch `feature/ds1-only-asymsoftclip-retune` retunes ONLY the existing DS-1 `asymSoftClip` knees in `hw/ip/clash/src/AudioLab/Effects/Distortion.hs`: `kneeP` `2_400_000 -> 1_900_000`, `kneeN` `2_000_000 -> 1_900_000`. This makes DS-1 fully symmetric and harder-edged while preserving the existing helper topology. No TS9 / RAT / Fuzz Face / Big Muff / Metal / clean boost / Overdrive / Amp / Compressor / Pipeline / LowPassFir / GUI / HDMI / Pmod RTL / block design source was edited. `asymSoftClip` call count remains `3`, `asymHardClip` remains `0`, and `mulU8` / `mulU12` counts remain `13` / `8`. Clash VHDL was regenerated, the Vivado bitstream was rebuilt, and the build completed with `write_bitstream completed successfully` and `0 Errors`. Routed timing: WNS `-8.016 ns`, TNS `-9648.033 ns`, WHS `+0.051 ns`, THS `0.000 ns`; main `clk_fpga_0` setup group has `2242` failing endpoints, worst path `ARG__7__1/CLK -> ds1_5_reg[1032]/D`. Utilization: LUT `19712`, FF `22160`, BRAM `6`, DSP `83`. bit/hwh md5 are `52f0e9937993dca11272d561f6cf6b32` / `d75d38394a529ac3524e0a64f73bcd34`. Python py_compile, 91 unittest cases, `tests/test_overlay_controls.py`, deploy-time board md5 checks, `AudioLabOverlay(download=True)`, ADC HPF True, VERSION `0x00480001`, MODE 3 mute, and `scripts/diagnose_pmod_loopback.py` all passed. The self-loopback smoke produced no QUANT! / STAIR! indicators, but remains only a structural smoke check; the user then requested merge to `main`, which is the acceptance signal for this D66 build. D66 is now the deployed source-control baseline; D62 remains the deeper rollback reference.
...
- **Latest PL timing baseline**: D66 WNS `-8.016 ns`, TNS
  `-9648.033 ns`, WHS `+0.051 ns`, THS `0.000 ns`; Slice LUTs
  `19712`, Slice Registers `22160`, BRAM `6`, DSPs `83`. The older
  Phase 6I HDMI-only timing baseline was WNS `-8.096 ns`; D66 is the
...
1. **D66 / Pmod mode-2 post-deploy QA.** The current active audio path is
   Pmod I2S2 mode 2 (`ADC -> DSP -> DAC`) with D50 mono RIGHT-slot
   mirroring, the D62 BD-2 coefficient-only retune, and the D66
   DS-1-only symmetric knee retune. Continue bench
...
5. **Drive WNS toward 0.** The deployed build is at the value
   recorded in `TIMING_AND_FPGA_NOTES.md` (D66 WNS `-8.016 ns`);
...
- Do **not** deploy a bitstream whose WNS is significantly worse than
  the current D66 baseline (`-8.016 ns`) without flagging the regression
