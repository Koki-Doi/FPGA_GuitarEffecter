...
**実際の per-model 定数（`Overdrive.hs` より、D150 適用後）** — この数値の組だけで
6 機種を弾き分けています（`odClipHardness` の 4 = `symSoftClipMed`、3 = `asymSoftClipHard`
はいずれも**対称スロープ**）：
...
| 1 OD-1 | 5 | 2,550,000 | 2,400,000 | 3,050,000 | 4 (対称) | D150 で対称化（kneeN 1.75M→2.4M、hardness 1→4） |
| 2 BD-2 | 7 | 2,400,000 | 2,300,000 | 3,400,000 | 4 (対称) | D150 で対称化（kneeN 1.9M→2.3M）、bite は mid biquad が担保 |
| 3 Jan Ray | 2 | 3,600,000 | 3,450,000 | 3,700,000 | 0 | トランスペアレント、D129 で +1.8 dB @ 350 Hz |
| 4 OCD | 7 | 2,450,000 | 2,350,000 | 3,750,000 | 3 (対称 hard) | D150 で対称化（kneeN 2.15M→2.35M、hardness 2→3）、MOSFET エッジ |
| 5 CENTAUR | 4 | 2,400,000 | 2,300,000 | 3,650,000 | 4 (対称) | D150 で対称化（kneeN 2.05M→2.3M）、グリットは clean blend |

`driveGain = 256 + (drive × k)` なので、DRIVE=255 で Jan Ray (k=2) は約 1〜2.99×、OCD (k=7) は
約 1〜7.97× に達します。**D150 のポイント**：ゲインの高い OD-1/BD-2/OCD/Klon は元々
**非対称スロープ**で強い偶数次（差音）を出し、和音で「歪が変／デチューン」に聞こえていました。
新ヘルパー `FixedPoint.symSoftClipMed`（pos>>2 neg>>2）と既存の対称 `asymSoftClipHard`
（pos>>1 neg>>1）に切り替え、`odKneeN` を `kneeP` 寄りに上げて、和音 IMD を**偶数次（差音）→
奇数次（協和的）**へ寄せました（placement-safe な定数のみ、bypass bit-exact、measure 28/28）。
TS9 / Jan Ray は元々ほぼ対称なので不変です。DS-1 も同様に対称クリップ化しています。
...
### 9.3 Distortion ペダルのモデル差（`Distortion/Pedals/*` ほか）
...
**各ペダルの実定数（`Distortion/Pedals/*` より抜粋。refactor K で 1 ペダル 1 モジュールに分割、`Pedals.hs` は re-export shim）** — どれも `mul → clip → tone → level` の小チェーンで、
...
短い early/body 係数、**47-tap 折返し speaker-rolloff FIR**（D149 で 15→31、D155 で
31→47 タップへ拡張＝real-4x12 寄りの >5 kHz ロールオフ open -14.9 / brit -22.0 / closed
-28.9 dB/oct。高リスクな 128-tap BRAM "B2" は不採用）、per-model cone-breakup presence
biquad（D123、D151 で +3 dB → D152 で +4.5/5/5.5 dB に戻す）、プレゼンス `softClipK`、
fizz 残差減算、ボディ強調で「ギタースピーカーは 5 kHz 以上でロールオフしつつ、2〜4 kHz に
切り込む presence peak を持つ」特性を再現しています（presence/body knee は D152 で
ヘッドルームを上げ和音の帯域内 IMD を低減）：

| cab | 直接/ボディ比 | プレゼンス knee（D152） | fizz 減算 | ボディ強調 | 参照スピーカー |
| --- | --- | --- | --- | --- | --- |
| 0 OPEN BACK | 2.76:1（直接優勢） | `softClipK(4.6M)` 25% | 12.5% | 0% | Fender 系 |
| 1 BRITISH | 1.24:1（バランス） | `softClipK(4.0M)` 12.5% | 25% | 6.25% | Vox 系（Celestion V30） |
| 2 CLOSED | 0.42:1（ボディ優勢） | `softClipK(3.3M)` 12.5% | 50% | 12.5% | Marshall/Mesa 系 |
...
今も rejected のままですが、現行 D155 の Amp source は D99/D121 untouched ではありません。
...
| D122 / D128 / D130 / D135 | 既存 ampScoop / presence / drive bonus / darken / treble trim / MIDDLE / power-headroom の measured retune | accepted Amp line（D155 にも含まれる） |
| D147 / D148 | amp sag-attack slew（chord-IMD 修正）+ JC-120 / Fender-Twin clean-headroom fix（`ampPowerKnee` / `ampCleanKneeBonus`） | accepted Amp line |
| D151 / D153 | post-amp HF シェルフ（`ampHfShelfFrame`、JC-120 除外）+ JC/Twin master -1.3 dB トリム（D152 cab headroom が出力をホット化した分の補正） | 現行 D155 Amp の accepted line |
| D154 | ゲイン機の和音 IMD 低減を multiband 含め試行 | 非改善（AC30 悪化）で未採用＝Drive つまみが制御 |
...
**実装（`Distortion/Common.hs` ＋ `AudioLab.FixedPoint` の共有ヘルパー）**：
