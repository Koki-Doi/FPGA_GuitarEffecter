...
- 構成：`drive mul → boost → モデル別硬さクリップ → tone → level`
...
- **15-tap 対称 FIR**（D87、`mulS10` 係数）によるスピーカーロールオフ + 微小モジュレーション
...
| metal | `metalHpf(tight) → metalMul → metalClip(hard) → metalPostLpf → metalLevel` |
| ds1 | `ds1Hpf → ds1Mul → ds1Clip(asym, 低い膝) → ds1Tone → ds1Level` |
| big_muff | `bigMuffPre → bigMuffClip1 → bigMuffClip2(2 段カスケード) → bigMuffTone → bigMuffLevel` |
...
| Cab | `cabProducts(4×mulS10) → cabIr → cabLevelMix` |
...
toneScaled = (tone - tone>>4) >> 1               -- TONE による一次 LPF ダンピング（D114 で最も明るい端を緩和）
...
| 関数 | 役割 |
| --- | --- |
| `odDriveK :: Unsigned 3 -> Unsigned 11` | モデル別ゲイン上限係数。`driveGain = 256 + (drive * k)`。例：Jan Ray `k=2`（DRIVE=255 で ~1〜3x）、OCD `k=8`（~1〜9x） |
...
OD-1 / OCD / Centaur には既存のミッド・ピーキング・バイカッド（96 kHz RBJ Q14 係数）も
適用して質感を分けます。
...
| --- | --- | --- | --- | --- | --- | --- |
| 0 TS9 | 5 | 2,850,000 | 2,700,000 | 3,300,000 | 0 (最柔) | オペアンプ・ソフト、ほぼ対称 |
| 1 OD-1 | 6 | 2,450,000 | 1,650,000 | 3,000,000 | 1 | 早く粗く、強い非対称 |
| 2 BD-2 | 7 | 2,400,000 | 1,900,000 | 3,350,000 | 1 | 偶数次最大（P/N gap 500k） |
| 3 Jan Ray | 2 | 3,600,000 | 3,450,000 | 3,800,000 | 0 | トランスペアレント |
| 4 OCD | 8 | 2,300,000 | 2,050,000 | 3,850,000 | 3 (最硬) | MOSFET 風ハード、ヘッドルーム大 |
| 5 CENTAUR | 3 | 2,550,000 | 2,100,000 | 3,700,000 | 1 | ゲルマ wet + クリーンブレンド |

`driveGain = 256 + (drive × k)` なので、DRIVE=255 で Jan Ray (k=2) は約 1〜3×、OCD (k=8) は
約 1〜9× に達します。`kneeN < kneeP` の差分（例：OD-1 は 800k、BD-2 は 500k）が偶数次倍音
（チューブ的な 2 次）の量を決めます。
...
| 共有トーンスタック・バイカッド | RBJ 設計の Q14 固定小数点。Rockerverb は 500 Hz を +3 dB、TriAmp は 750 Hz を -3 dB スクープ（D113） |
| `ampPreEmph` / `ampDeEmph` | プリ/ディエンファシス（`ampEmphShift=4`、`ampEmphAmount=2`）で帯域を制御 |
...
| --- | --- | --- | --- | --- | --- |
| 0 JC-120 | 18 | 1 | 14,000 | 18 | 高ヘッドルーム SS クリーン |
| 1 Twin Reverb | 78 | 2 | 76,000 | 26 | グラッシーなチューブクリーン |
| 2 AC30 | 166 | 3 | 250,000 | 48 | 早めのチャイム・ブレイクアップ |
| 3 Rockerverb | 208 | 11 | 410,000 | 68 | 太く暗い EL34 サチュレーション |
| 4 JCM800 | 220 | 6 | 505,000 | 82 | 噛みつくクラシックロック |
| 5 TriAmp Mk3 | 246 | 13 | 680,000 | 92 | タイトなモダンハイゲイン |
...
| 段 | 処理内容 |
| --- | --- |
| `ampHighpassFrame` | 1 次 HPF。フィードバック係数 `253/256`（D109 で dead-pole 修正、ライブな極に） |
...
| `ampMasterFrame` | master 乗算 + `softClipK 3_300_000`（MASTER が Cab/EQ/Reverb をハードクリップに叩き込まない保護） |
...
- **DS-1**：非対称ソフトクリップの膝チューニング（D63/D64 の経緯から D114 では不変に保持）
- **Big Muff**：多段クリップカスケード + 約 700 Hz のミッドスクープ・ノッチ（D82）+ **4x オーバーサンプル**（D90）
...
| big_muff | `448 + drive×11` | 2 段カスケード 2,400,000 → ×208 → 1,850,000 | `25 + tone>>2` | softClipK | 厚いファズ + ミッドスクープ |
| metal | `768 + drive×12` | hardClip（閾値 `3,300,000 − drive×6,000`、下限 1,450,000） | post-LPF | softClip | 最もタイト・高ゲイン |
...
**Big Muff のミッドスクープ**は一点もの：一次 LPF では「暗くする」ことしかできず**ミッドを抉れ
ない**ため、飽和後の信号に**負ゲインのピーキング・バイカッド**（`bigMuffScoopFeedforward` +
`bigMuffScoopRecursive`）を掛けて約 700 Hz の谷を彫り出しています（D82）。
...
4-tap FIR + プレゼンス `softClipK` + fizz 残差減算 + ボディ強調で「ギタースピーカーは 5 kHz
以上で急峻にロールオフする」特性を再現しています：
...
**BRAM も IR ローダも使わず、LUT/DSP 内で完結**しているのがこの方式の利点です。

### 9.5 アンプ全面リボイス（D110-D112、現ベースライン）の具体例

D109 で入力 HP の dead-pole（`prevOut × (509>>9) == ×0` ＝ 単なる 1 次差分）を修正したことで、
それまで「明るい微分器入力」を前提にしていた D55-D97 のアンプ voicing が「こもって潰れた」
状態になりました。D110-D112 はフラット入力に合わせて以下を**定数のみ**で開け直しています：

- 常時 ON のカスケード・ソフトクリップ膝を緩和：power `3.4M→6.0M`、master `3.3M→4.5M`、
  midsat `4.0M→6.5M`、respres `3.4M→5.5M`、transformer `5.2M→6.5M`（＝圧縮↓・トランジェント復活）
- post-clip LPF の HF 天井 `ampPreLowpass` baseAlpha `80→140`（約 6 kHz → 約 12 kHz）
- treble 完全伸長（`ampTrebleGain` の top-rolloff 撤去）、transformer HF droop `3→6`（≒オフ）
- 非対称クリップ基準膝 `4.9M/4.35M → 5.5M/4.9M`、pre/de-emph 量 `1→2`
- ただし master 天井は保護的に `4.5M` を維持（JC-120 クリーンが satShift7 でオーバーフローしない）

結果は実機試聴で「all_off クリーン、アンプは自然で開放的・トップが伸び、JC-120 クリーン、
チューブ系のこもり解消」＝**合格**。これが現行ベースライン `c1e3de50`（D112）です。

### 9.6 4x オーバーサンプリング（アンチエイリアス）
...
- Big Muff は単純な 1 段クリップではなく **`bigMuffOsCascade`**（`softClipK 2,400,000` → ×208 →
  `softClipK 1,850,000` の 2 段カスケード）を各サブサンプルに適用し、`Vec 16 Sample` の履歴を
  `os4xHistShift` で回します
