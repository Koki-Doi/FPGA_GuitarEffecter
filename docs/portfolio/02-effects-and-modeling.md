...
TS9 は 720 Hz の mid hump、BD-2 は D121 で 2300 Hz の bright upper-mid、OCD は D121 で
1300 Hz の upper-mid honk を持ち、それ以外は `b0=2^14` の flat unity です。モデルごとに
...
| 2 BD-2 | 7 | 2,400,000 | 1,900,000 | 3,400,000 | 1 | 偶数次最大（P/N gap 500k）、D121 で +3.5 dB @ 2300 Hz |
| 3 Jan Ray | 2 | 3,600,000 | 3,450,000 | 3,700,000 | 0 | トランスペアレント、pre-clip は flat |
| 4 OCD | 7 | 2,450,000 | 2,150,000 | 3,750,000 | 2 | MOSFET 風 hard-leaning、D121 で +4 dB @ 1300 Hz |
| 5 CENTAUR | 4 | 2,400,000 | 2,050,000 | 3,650,000 | 1 | ゲルマ wet + クリーンブレンド |
...
| 共有トーンスタック・バイカッド | RBJ 設計の Q14 固定小数点。JC-120 / Twin は 400 Hz Fender-style scoop、AC30 は 2200 Hz chime、JCM800 は 650 Hz mid。Rockerverb / TriAmp は現行 accepted baseline では flat |
...
| 1 Twin Reverb | 78 | 12 | 85,800 | 30 | グラッシーなチューブクリーン |
| 2 AC30 | 166 | 17 | 232,400 | 42 | 早めのチャイム・ブレイクアップ |
| 3 Rockerverb | 208 | 31 | 374,400 | 62 | 太く暗い EL34 サチュレーション |
| 4 JCM800 | 220 | 25 | 462,000 | 74 | 噛みつくクラシックロック |
| 5 TriAmp Mk3 | 246 | 39 | 615,000 | 88 | タイトなモダンハイゲイン |
...
| 段 | 処理内容 |
| --- | --- |
| `ampHighpassFrame` | 1 次 HPF 形。現行 D121/D99 Amp では `onePoleHighpass 509 9` の係数丸めにより実質 `x - prevIn` に近い挙動を保持 |
...
現行 D121 では、**`Amp.hs` 系の source は触っていません**。D121 の意義は、D99 系の Amp
キャラクターを保持したまま、外れていた非 Amp 帯域だけを measured pass で直した点です。
したがってポートフォリオで Amp を語るときは、次のように分けて説明するのが正確です。

| 系譜 | 状態 | 扱い |
| --- | --- | --- |
| D99 / D121 Amp | 現行 accepted character。D121 では Amp source untouched | 現行の音の前提 |
