...
- GUI のノブ/エフェクト切替を**ライブ反映**（dirty-flag ループ、poll 60 Hz アクティブ /
  10 Hz アイドル、レンダ上限 20 fps、live-apply 20 ms スロットル）
- エンコーダ 0 トグルは `button_state` 立ち上がり **OR** HW `short_press` ラッチ
  （poll 周期より短いタップを取りこぼさない）
- standalone runner は RAT を既定で含める（`--include-rat` 相当）。ライブラリ安全側の
  `EncoderEffectApplier(skip_rat=True)` default とは分け、実運用 entry point では
  Distortion model cycling から RAT bit2 → dedicated RAT stage へのルートも扱う

### 10.3 フットスイッチ ×3
- `axi_footswitch_input`（`0x43D50000`）で FS1=FX トグル / FS2=プリセット次 / FS3=前
- `scripts/run_encoder_hdmi_gui.py` では footswitch polling が**既定 ON**。古い bit などで
  IP が存在しない場合は初期化失敗を表示して続行し、診断時だけ `--no-footswitch` で明示的に切る
- FS1 を 3 秒以内に 5 回 stomp すると、FS1 の対象エフェクトを現在 GUI で選択中の effect に rebind
...
### 10.5 notebook-less HDMI / encoder runtime

実運用の中心は `scripts/run_encoder_hdmi_gui.py` です。これは Jupyter Notebook に依存せず、
AudioLab overlay、HDMI backend、encoder input、live-apply、FP02M pedal、footswitch を 1 プロセスで
束ねます。

```bash
sudo env PYTHONPATH=/home/xilinx/Audio-Lab-PYNQ python3 \
  /home/xilinx/Audio-Lab-PYNQ/scripts/run_encoder_hdmi_gui.py \
  --pmod-mode dsp \
  --wah-pedal
```

既定値は、`--live-apply` ON、`--apply-interval-ms 20`、`--poll-hz-active 60`、
`--poll-hz-idle 10`、`--max-render-fps 20`、`--footswitch` ON、RAT include です。
`--pmod-mode dsp` を指定すると起動時に Pmod I2S2 MODE=2 を書き、Ctrl+C / SIGTERM で終了すると
MODE=3（mute）を書いて外部スピーカーを黙らせます。`--pmod-mode keep` の場合は Pmod register に
触りません。

overlay bring-up は smart attach です。`pynq.PL.bitfile_name` がすでに `audio_lab.bit` なら
`AudioLabOverlay(download=False)` で attach し、rgb2dvi PLL を不用意に再ロックさせません。
`base.bit` や別 overlay が載っている場合だけ、通常の `AudioLabOverlay()` で full download します。
この runner は `Overlay("base.bit")` を呼ばず、AudioLab overlay を 2 回ロードしません。

診断用の代表オプション：

| オプション | 用途 |
| --- | --- |
| `--no-audio-apply` | GUI/encoder の状態更新だけ行い、`AudioLabOverlay.set_*` 書き込みを止める |
| `--dry-run` | overlay / HDMI / encoder bring-up を完全にスキップして off-board CLI smoke に使う |
| `--no-live-apply` | encoder 3 short press 時だけ apply。連続 apply を止めて原因切り分けする |
| `--skip-rat` | RAT を encoder model cycling / live apply から外す |
| `--no-footswitch` | footswitch polling を止める |
| `--wah-pedal-debug` / `--footswitch-debug` | pedal / footswitch の入力状態を標準出力へ出す |
...
Clash ソース (LowPassFir.hs / AudioLab/Effects/*.hs)  ← DSP の唯一の真実
   → tools/dsp_sim で offline sim / bypass invariant / net tone-curve 測定
   ...
   → scripts/deploy_to_pynq.sh で実機へ（必要コピー先の bit/hwh を md5 同期）
   → プログラム的スモークテスト（ADC HPF, Pmod VERSION/MODE, I2S クロック生存）
   → 実機試聴（ear-bench）で「合格」したらベースライン昇格
```

- **offline sim first**：DSP voicing 変更は `tools/dsp_sim/Sim.hs` で Clash `topEntity` を host CPU 上で
  実行し、`measure.py` で bypass 比の net tone-curve を測る。D121 では BD-2 / OCD / Metal /
  Cab の 4 変更をここで確認してから Vivado へ進んだ
- **複数コピー先同期**：bit/hwh はリポジトリ / `dist-packages/audio_lab_pynq` /
...
```bash
# DSP offline sim（Vivado 前の音作り測定）
...
python3 tools/dsp_sim/measure.py --help
