...
3. **「ナイフエッジ」バグの根本原因究明（CDC デバッグの白眉）**
   ...
   `set_max_delay -datapath_only` で拘束（D109）。再ビルド成功率は大幅に改善したが、
   D136-D144 で placement sensitivity が残ることも確認し、実機試聴 gate を維持している。
...
| 採用ベースライン（D155）の WNS | +0.319 ns（WHS +0.013 ns、route errors 0、D109 CDC fwd +0.989） |
| エフェクト段数 | 9 ブロック（内部はさらに多段カスケード） |
| Amp モデル | 6 |
| Overdrive モデル | 6 |
| Distortion ペダル | 7（マスク方式、bit7 予約） |
| Cab IR | 3 プリセット、47-tap 折返し speaker-rolloff FIR（D155、D149 で 15→31→47） |
...
**現行 D155 bit/hwh**：`8d875cc8a0154a86673ab22e5b142d27` /
`e0469cf593e97d582c14bb09ea98d3d3`。小さなプロトタイプではなく、HDMI / input IP /
...
| D109 | **safe-bypass ナイフエッジの根本究明＆拘束追加**（未タイミング CDC。placement sensitivity は残存） |
...
| D131 | DIST low-end + saturation/sustain + `dist_eval.py` / `targets.py` |
| D134 | 全 effect dynamics / knob audibility の objective check を拡張 |
| D135 | Fuzz Face mid-hump + Amp/Cab character、bench-ACCEPTED |
| D136-D144 | Amp clean / chord-detune 候補。sim/timing は改善したが safe-bypass を含む bench で rejected、D135 へ rollback |
...
| D148 | JC-120 / Fender-Twin clean-headroom fix（`clip_onset.py` で局在化）、bench-ACCEPTED、D135 を supersede |
| D149 | cab real-IR step B1 = 31-tap rolloff-only speaker FIR（Option Y、real-4x12 寄りの >5 kHz ロールオフ） |
| D150 | OD/DS の和音 IMD = 非対称クリップの偶数次差音を対称クリップ（`symSoftClipMed`）で修正 |
| D151 | amp 高音増強 = post-amp HF シェルフ + cab presence。rig の高域は cab が支配し amp の TREBLE/PRESENCE はほぼ効かないと実測で確定 |
| D152 / D153 | 和音高音の "汚い/ブツブツ" = 帯域内 IMD（OS 無効と実証）。cab ヘッドルーム + presence 戻し → 出力ホット化分の `cabSpeakerKnee` peak-limit 復帰 + JC/Twin -1.3 dB（音割れ修正） |
| D154 | ゲイン機の和音 IMD 低減を multiband 含め試行 → 非改善（AC30 悪化）で**未採用**＝Drive つまみが制御 |
| D155 | **現行 canonical baseline**。cab speaker FIR を 31→47 タップへ拡張（real-4x12 寄りロールオフ）、bench-ACCEPTED、D153 を supersede |
...
です。D121 当時は D99 系 Amp character を保持していたため、この節は「現行 D155 Amp がこうなっている」
...
### 18.9 挙動保存リファクタリング — 「等価を証明してから統合」

肥大化したモジュールを安全に分割・共通化するために、**挙動を保存したまま等価を機械的に証明する**
規律を確立した。

- **golden 回帰 / ピクセル digest をゲートに**：DSP リファクタは `tools/dsp_sim` の golden 20/20
  （bypass＋全 6 アンプ＋7 歪み＋OD がバイト一致）、GUI レンダラ分割は 108 フレームの
  バイト digest（states×themes×variants×cache 有無）で**バイト等価を確認**してからマージ。
- **F-pattern**：Clash ソースをリファクタしても、再合成ビットが機能同一なら **`vhdl/bit` は
  accepted baseline（D155）に据え置く**。新ビットを焼かない＝このマージに耳試聴は不要。次に
  ソースからビットを再生成するときに通常の timing/CDC/ear-bench を通す、と切り分ける。
- **適用例**：クリップ族 5 関数を 1 つの `softClipShift` に（J）、`Distortion/Pedals.hs` 583 行を
  1 ペダル 1 モジュールに（K）、共鳴 biquad を `biquadFf`/`biquadRec`/`biquad5` に集約（B）、
  レンダラの per-panel 分割＋共有キャッシュホルダ（P2、`renderer.py` 1048→536 行）、HDMI mirror の
  reporting 抽出（M）、`apply_chain_preset` の純粋 kwargs ビルダ抽出（P1）。
- **「抽出して終わり」にしない監査**：残った `_apply_*` / orchestration メソッドは複数の
  インスタンス属性とオーバーレイ書き込みを同時に行う**真のクラス結合**なので、無理に剥がすと
  `self` を引き回すだけで可読性が下がる、と判断して止めた（M / P1 で明示）。
