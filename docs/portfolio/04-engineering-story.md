...
4. **offline DSP sim による音作りループの高速化**
   ...
   golden regression / net tone-curve を Vivado 前に確認できるようにした。D121 では BD-2 / OCD /
   Metal / Cab の 4 変更をこの経路で測定し、build 前に狙いの周波数へ着地していることを確認。
...
| 採用ベースライン（D121）の WNS | +0.726 ns（TNS 0、WHS +0.012 ns、route errors 0） |
| D121 DSP island slack | `clk_fpga_1` WNS +3.309 ns |
...
**D121 FPGA リソース使用（Zynq-7020）**：Slice LUT `30,792`（57.88%）、
Slice Register `28,896`（27.16%）、BRAM tile `6`（4.29%）、DSP48E1 `142`（64.55%）。
小さなプロトタイプではなく、HDMI / input IP / Pmod I2S2 / full DSP chain を統合した状態で、
Zynq-7020 に収まっています。**モデルを増やしても不要に DSP48 数を増やさない**
...
| D121 | **offline sim 測定駆動の非 Amp リボイス**。D99 Amp を保持し、BD-2 / OCD / Metal / Cab を補正。現行 canonical baseline `9a57c50a` |
...
BD-2 / OCD / Metal / Cab の変更は、まず `measure.py` で「狙った帯域が本当に動いたか」を確認し、
golden / bypass invariant を通してから bitstream 化しています。これにより、30〜40 分級の Vivado
build を「試し打ち」に使わずに済みます。

ただし sim は DSP island の論理だけを動かします。Pmod I2S2、CDC、routing、PLL、実アンプでの
弾き心地は見えません。したがって正しい使い方は、**sim で候補を落とし、Vivado / smoke / bench
で採用を決める**ことです。D121 が通ったのは、この分担を守ったからです。
...
です。現行 D121 は D99 系 Amp character を保持しているため、この節は「現行 Amp がこうなっている」
