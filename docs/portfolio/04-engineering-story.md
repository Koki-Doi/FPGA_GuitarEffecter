...
| 採用ベースライン（D148）の WNS | +0.526 ns（WHS +0.014 ns、route errors 0） |
...
**現行 D148 bit/hwh**：`972d9ba6645dd966e6bdcb5bc3daf478` /
`2b888ff1ec3168cd64e1b679bbbc71be`。小さなプロトタイプではなく、HDMI / input IP /
...
| D135 | Fuzz Face mid-hump + Amp/Cab character、bench-ACCEPTED（D148 まで rollback target） |
...
| D148 | **現行 canonical baseline**。JC-120 / Fender-Twin clean-headroom fix（`clip_onset.py` で局在化）、bench-ACCEPTED（「完璧」）、D135 を supersede |
...
です。D121 当時は D99 系 Amp character を保持していたため、この節は「現行 D148 Amp がこうなっている」
