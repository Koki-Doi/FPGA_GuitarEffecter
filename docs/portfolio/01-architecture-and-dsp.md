...
現行の採用済みデプロイベースラインは **D131** です。これは D109 の CDC 修正で DSP
再ビルド時の safe-bypass 破壊を根本解決したあと、D121-D131 の測定駆動 voicing line
（Cab / OD / Distortion / Compressor / Amp / offline distortion metrics）を通して
...
### 3.1 現行採用ビルド（D131）の実測値

| 項目 | 値 |
| --- | --- |
| 採用日 | 2026-06-15 |
| Git / bitstream | merge commit `37114b9`、bit md5 `fdab62d5ef229ec64dc60fe9395cbf06`、hwh md5 `d852ec4e737460ad016b41f0a3f71de2` |
| タイミング | WNS `+0.631 ns`、WHS `+0.019 ns`、route errors `0` |
| D109 CDC slack | `clk_fpga_0 -> clk = +3.353 ns`、`clk -> clk_fpga_0 = +6.286 ns` |
...
| ロールバック | D130 bitstreams を `fffa2b1` から復元（bit md5 `33af82f131cfff260871599e5142ef59`）して deploy |

この表は「ポートフォリオ用に見栄えよく丸めた値」ではなく、D131 の Vivado routed timing /
