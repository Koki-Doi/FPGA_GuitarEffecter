...
#     http://192.168.1.9:9090/tree/audio_lab/HdmiGuiShow.ipynb
...
ssh xilinx@192.168.1.9 '
  ...
  sudo env PYTHONPATH=/home/xilinx/Audio-Lab-PYNQ \
    python3 scripts/run_encoder_hdmi_gui.py --live-apply --include-rat --pmod-mode dsp --wah-pedal
'
...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build は **D155**
（merge `09c8a95`、cab speaker FIR を 31→47 タップに拡張 = Option-Y 折返し拡張で
real-4x12 寄りの >5 kHz ロールオフを鋭く。presence biquad が 2-4 kHz ピークを保つ
ので D151 の明るさは不変。高リスクな 128-tap BRAM "B2" ではない）で、`.bit =
8d875cc8a0154a86673ab22e5b142d27`、`.hwh =
e0469cf593e97d582c14bb09ea98d3d3` です。build MET WNS +0.319、D109 CDC fwd
+0.989（bench-clean）、DSP 206/220、bypass bit-exact。**2026-06 の D150-D155
voicing アーク**（全て sim 先行 `tools/dsp_sim`・placement-safe）: D150 OD/DS の
和音 IMD（非対称クリップの偶数次差音）を対称クリップで修正、D151 amp の高音増強
（post-amp HF シェルフ + cab presence。rig の高域は cab が支配し amp の
TREBLE/PRESENCE はほぼ効かない）、D152/D153 和音高音の "汚い/ブツブツ"（帯域内
IMD、オーバーサンプリング無効と実証）を cab ヘッドルーム + presence 戻し → 出力が
ホット化した分 `cabSpeakerKnee` peak-limit を戻し JC/Twin を -1.3 dB トリム
（音割れ修正）、D154 ゲイン機の和音 IMD はマルチバンド含め非改善＝未出荷、D155 が
上記 cab 47-tap。`scripts/deploy_to_pynq.sh` は無 download の bit/hwh md5 整合性
チェックを追加（deploy は PL をロードしない。`download=True` の反復は board を
ハングさせる）。直前の accepted は D153（`b86c88a`）、その前は D148（`96ef899`）、
さらに前は D135（`765323b`）。D153 へ戻す場合は
`git checkout b86c88a -- hw/Pynq-Z2/bitstreams/` で bit/hwh を復元して
redeploy します。履歴上の baseline 一覧は
`docs/ai_context/BASELINES.md` を参照してください。
...
top-level EFFECTS ではなく **Distortion 内 pedal-mask bit 2** です。ライブラリ
default は安全側 (`skip_rat=True`) ですが、現行 standalone runtime は既定で RAT を含めます
(`--include-rat`)。
...
```text
http://192.168.1.9:9090/tree/audio_lab/EncoderGuiSmoke.ipynb
```
...
| `--skip-rat` / `--include-rat` | `--include-rat` | RAT (Distortion pedal-mask bit 2) を encoder cycle と live apply に含める。診断時は `--skip-rat` で除外 |
...
- RAT pedal-mask bit 2 は現行 standalone runtime では encoder 操作対象に含まれます。
  `--skip-rat` を指定すると従来どおり除外できます。Clash stage と
  `HdmiEffectStateMirror.rat()` は手付かずで、notebook からの RAT 直接呼び出しは
  従来通り動作します。
...
D58.2 の fixed-scalar 方式が現在も load-bearing な方針です。係数は
D128 / D130 の bench-accepted amp realism pass で更新されています。
...
現行の D155 build はこの DSP island 方針を保持したまま、D89 / D94 で
FCLK1 を `40 MHz` から `33.33 MHz` へ下げ、D76 XADC、D78 footswitch、
D79 以降の realism pass、D131 の DIST tooling、D134 の全 knob 評価、
D135 の Fuzz Face / Amp / Cab character 強化、D146 hard CDC pblock、D147 amp
sag-attack slew、D148 JC/Twin clean-headroom fix を重ねています。D148 の routed
timing は `WNS = +0.526 ns`、`WHS = +0.014 ns` で、実機 bench accepted（「完璧」）
です。D136-D144 で再確認した通り、この timing 数値だけでは safe-bypass の
合否を予測できないため、各新 bitstream は必ず実機試聴します。
...
| `PmodI2S2HdmiGuiOneCell.ipynb` | HDMI GUI + rotary encoder / footswitch runtime を Pmod I2S2 mode 2 で起動する 1 セル Notebook。`scripts/run_encoder_hdmi_gui.py --live-apply --include-rat --pmod-mode dsp --wah-pedal` を sudo subprocess として起動し、停止時は mute (`MODE=3`) |
...
| `Pcm5102DspOutputCheck.ipynb` | retired PCM5102 output path の履歴診断用。現行 PMOD JB audio path は Pmod I2S2 |
| `AudioLab.ipynb` | 元の Audio Lab 操作用ノートブック |

PYNQ 上では通常、次のURLから開きます。

```text
http://<PYNQのIPアドレス>:9090/tree/audio_lab/PmodI2S2EffectControlOneCell.ipynb
```
...
フォルダ一覧は `http://192.168.1.9:9090/tree/audio_lab`、メイン Notebook は
`http://192.168.1.9:9090/tree/audio_lab/AudioLab.ipynb` です。deploy helper
は `jupyter notebook list` から実際の server root を検出し、15 件すべてを
JSON 検証して `xilinx:xilinx` 所有に戻します。
...
Jupyter     : http://192.168.1.9:9090/tree/audio_lab
...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build (D155) の
md5 は `8d875cc8a0154a86673ab22e5b142d27` (`.bit`) /
`e0469cf593e97d582c14bb09ea98d3d3` (`.hwh`) です。
...
配置後は `/home/xilinx/jupyter_notebooks/audio_lab` を `xilinx:xilinx` 所有に
戻し、全 15 Notebook が JSON として読めることを確認してください。通常の
deploy helper はこの検査を自動実行します。ブラウザの直接 URL は
`http://192.168.1.9:9090/tree/audio_lab`、メイン Notebook は
`http://192.168.1.9:9090/tree/audio_lab/AudioLab.ipynb` です。
...
- 最新の deploy 済 bitstream は D155 cab speaker FIR 31->47 タップ build（D150-D155
  voicing アークの締め、D153 を supersede）で、
  `WNS = +0.319 ns`、`WHS = +0.013 ns`、bit/hwh md5 は
  `8d875cc8...` / `e0469cf5...` です。
  D58 / D59 / D60 / D61 / D63 / D64 /
  D74 / D78 に加え、D136-D144 の rejected / noisy attempts で分かった通り、
  WNS や CDC slack だけでなく safe-bypass の実音確認も deploy gate です。
...
| [`DECISIONS.md`](docs/ai_context/DECISIONS.md) | 重要決定の付番ログ (D1..D145、その時々の状況 / 決定 / 境界 / why を記録) |
