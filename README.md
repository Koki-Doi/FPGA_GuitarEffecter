...
**2026-06-17 realism baseline (全エフェクト sim 調査 + bass / Metal 完全飽和 /
RESONANCE / こもり修正 / クリーン用パワーヘッドルーム + 包括 sim 検知器)** で、
`.bit = 54f7f547d04f0e4d59011e4754f834ca`、
`.hwh = 2fbc8a5ba528bb6e1d415e6339b64bdb` です。直前の b3dcab00 へ戻す場合は
`git checkout 55ef823 -- hw/Pynq-Z2/bitstreams/`(D131 なら `37114b9`)で
bit/hwh を復元して redeploy します。履歴上の baseline 一覧は
...
現行の D131 build はこの DSP island 方針を保持したまま、D89 / D94 で
FCLK1 を `40 MHz` から `33.33 MHz` へ下げ、D76 XADC、D78 footswitch、
D79 以降の realism pass、D131 の DIST realism / distortion-eval tooling
を重ねています。D131 の routed timing は `WNS = +0.631 ns`、
`WHS = +0.019 ns`、CDC max-delay slack は `clk_fpga_0 -> clk = +3.353 ns` /
`clk -> clk_fpga_0 = +6.286 ns` で、実機 bench accepted です。
...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build (D131) の md5 は
`fdab62d5ef229ec64dc60fe9395cbf06` (`.bit`) /
`d852ec4e737460ad016b41f0a3f71de2` (`.hwh`) です。
...
- 最新の deploy 済 bitstream は D131 DIST realism build で、
  `WNS = +0.631 ns`、`WHS = +0.019 ns`、CDC max-delay slack は
  `clk_fpga_0 -> clk = +3.353 ns` / `clk -> clk_fpga_0 = +6.286 ns` です。
...
| [`DECISIONS.md`](docs/ai_context/DECISIONS.md) | 重要決定の付番ログ (D1..D131、その時々の状況 / 決定 / 境界 / why を記録) |
