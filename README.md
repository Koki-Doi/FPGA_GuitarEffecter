...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build は **D135**
（merge `765323b`、Fuzz Face 900 Hz mid-hump、Amp/Cab character 強化などの
large non-IR realism pass）で、`.bit =
533d586901dc3669285a49c6d82bab9f`、`.hwh =
731517487c6218f0e181c2b74485d7a6` です。D136-D142 と D144 は bench
rejected のため、2026-06-19 に repo / board とも D135 へ rollback 済みです。
直前の accepted D134 へ戻す場合は
`git checkout f62f132 -- hw/Pynq-Z2/bitstreams/` で bit/hwh を復元して
...
現行の D135 build はこの DSP island 方針を保持したまま、D89 / D94 で
FCLK1 を `40 MHz` から `33.33 MHz` へ下げ、D76 XADC、D78 footswitch、
D79 以降の realism pass、D131 の DIST tooling、D134 の全 knob 評価、
D135 の Fuzz Face / Amp / Cab character 強化を重ねています。D135 の routed
timing は `WNS = +0.643 ns`、`WHS = +0.018 ns` で、実機 bench accepted
...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build (D135) の
md5 は `533d586901dc3669285a49c6d82bab9f` (`.bit`) /
`731517487c6218f0e181c2b74485d7a6` (`.hwh`) です。
...
- 最新の deploy 済 bitstream は D135 large non-IR realism build で、
  `WNS = +0.643 ns`、`WHS = +0.018 ns`、bit/hwh md5 は
  `533d5869...` / `73151748...` です。
