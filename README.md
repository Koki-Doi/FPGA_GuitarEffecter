...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build は **D148**
（merge `96ef899`、JC-120 / Fender-Twin clean-headroom fix = 演奏時のみの音割れ
修正、bypass は clean なので CDC ではない。新規 `tools/dsp_sim/clip_onset.py` で
局在化し、placement-safe な knee 定数のみで修正。merge は D146 hard CDC pblock
と D147 amp sag-attack slew も同梱）で、`.bit =
972d9ba6645dd966e6bdcb5bc3daf478`、`.hwh =
2b888ff1ec3168cd64e1b679bbbc71be` です。D135（`765323b`）を supersede します。
D136-D142 と D144 は bench rejected のため一旦 D135 へ rollback 後、D146 hard
pblock で crossing を物理固定してから clean-headroom voicing が landした流れです。
D135 へ戻す場合は
`git checkout 765323b -- hw/Pynq-Z2/bitstreams/` で bit/hwh を復元して
...
現行の D148 build はこの DSP island 方針を保持したまま、D89 / D94 で
...
5 行とも同じ md5 が出れば OK。最新の accepted deployed build (D148) の
md5 は `972d9ba6645dd966e6bdcb5bc3daf478` (`.bit`) /
`2b888ff1ec3168cd64e1b679bbbc71be` (`.hwh`) です。
...
- 最新の deploy 済 bitstream は D148 JC/Twin clean-headroom build（D146 hard CDC
  pblock + D147 sag slew 同梱、D135 を supersede）で、
  `WNS = +0.526 ns`、`WHS = +0.014 ns`、bit/hwh md5 は
  `972d9ba6...` / `2b888ff1...` です。
