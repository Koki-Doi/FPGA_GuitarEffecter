...
typedef struct {
    ...
    XFx_gain_uio_map maps[ MAX_UIO_MAPS ];
} XFx_gain_uio_info;

/***************** Variable Definitions **************************************/
static XFx_gain_uio_info uio_info;
...
static int uio_info_read_name(XFx_gain_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFx_gain_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFx_gain_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFx_gain_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFx_gain_Initialize(XFx_gain *InstancePtr, const char* InstanceName) {
	XFx_gain_uio_info *InfoPtr = &uio_info;
